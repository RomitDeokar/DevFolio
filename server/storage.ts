...
export interface IStorage {
  ...
  searchContent(query: string, type?: 'blog' | 'project'): Promise<(BlogPost | Project)[]>;
  ...
  // General
  getContentByCategory(category: string): Promise<(BlogPost | Project)[]>;
  incrementViews(type: 'blog' | 'project', slug: string): Promise<void>;
}
...
- Significant improvement in SEO rankings`,
    ...
    sampleProjects.forEach(project => {
      const id = randomUUID();
      const projectItem: Project = { ...project, id };
      this.projects.set(id, projectItem);
    });
  }
  ...
  async searchContent(query: string, type?: 'blog' | 'project'): Promise<(BlogPost | Project)[]> {
    const searchTerm = query.toLowerCase();
    const results: (BlogPost | Project)[] = [];

    if (!type || type === 'blog') {
      const blogResults = Array.from(this.blogPosts.values()).filter(post =>
    ...
    if (!type || type === 'project') {
      const projectResults = Array.from(this.projects.values()).filter(project =>
  ...
  async getContentByCategory(category: string): Promise<(BlogPost | Project)[]> {
    const results: (BlogPost | Project)[] = [];
    
    const blogResults = Array.from(this.blogPosts.values()).filter(
      post => post.category.toLowerCase() === category.toLowerCase()
    );
    const projectResults = Array.from(this.projects.values()).filter(
  ...
  async incrementViews(type: 'blog' | 'project', slug: string): Promise<void> {
    if (type === 'blog') {
      const post = Array.from(this.blogPosts.values()).find(p => p.slug === slug);
      if (post) {
        const currentViews = parseInt(post.views.replace(/,/g, '')) || 0;
        post.views = (currentViews + 1).toLocaleString();
      }
    } else {
      const project = Array.from(this.projects.values()).find(p => p.slug === slug);
      if (project) {
        const currentViews = parseInt(project.views.replace(/,/g, '')) || 0;
        project.views = (currentViews + 1).toLocaleString();
      }
    }
  }
}
